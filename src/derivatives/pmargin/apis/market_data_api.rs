...
/// struct for typed errors of method [`pmargin_get_ping_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PmarginGetPingV1Error {
...
pub async fn pmargin_get_ping_v1(configuration: &configuration::Configuration) -> Result<serde_json::Value, Error<PmarginGetPingV1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<PmarginGetPingV1Error> = serde_json::from_str(&content).ok();
