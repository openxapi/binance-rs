...
use crate::derivatives::futuresdata::{apis::ResponseContent, models};
use super::{Error, configuration, ContentType};

/// struct for passing parameters to the method [`futuresdata_get_futures_hist_data_link_v1`]
#[derive(Clone, Debug, Default)]
pub struct FuturesdataGetFuturesHistDataLinkV1Params {
...
/// struct for typed errors of method [`futuresdata_get_futures_hist_data_link_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FuturesdataGetFuturesHistDataLinkV1Error {
...
pub async fn futuresdata_get_futures_hist_data_link_v1(configuration: &configuration::Configuration, params: FuturesdataGetFuturesHistDataLinkV1Params) -> Result<models::FuturesdataGetFuturesHistDataLinkV1Resp, Error<FuturesdataGetFuturesHistDataLinkV1Error>> {
    ...
    query_params.push(("dataType".to_string(), params.data_type.to_string()));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FuturesdataGetFuturesHistDataLinkV1Resp`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FuturesdataGetFuturesHistDataLinkV1Resp`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FuturesdataGetFuturesHistDataLinkV1Error> = serde_json::from_str(&content).ok();
