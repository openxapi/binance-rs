...
/// struct for passing parameters to the method [`cmfutures_get_pm_account_info_v1`]
#[derive(Clone, Debug, Default)]
pub struct CmfuturesGetPmAccountInfoV1Params {
    pub asset: String,
    pub recv_window: Option<i64>
}


/// struct for typed errors of method [`cmfutures_get_pm_account_info_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CmfuturesGetPmAccountInfoV1Error {
...
pub async fn cmfutures_get_pm_account_info_v1(configuration: &configuration::Configuration, params: CmfuturesGetPmAccountInfoV1Params) -> Result<models::CmfuturesGetPmAccountInfoV1Resp, Error<CmfuturesGetPmAccountInfoV1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CmfuturesGetPmAccountInfoV1Resp`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CmfuturesGetPmAccountInfoV1Resp`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CmfuturesGetPmAccountInfoV1Error> = serde_json::from_str(&content).ok();
