...
/// struct for passing parameters to the method [`umfutures_get_pm_account_info_v1`]
#[derive(Clone, Debug, Default)]
pub struct UmfuturesGetPmAccountInfoV1Params {
    pub asset: String,
    pub timestamp: i64,
...
/// struct for typed errors of method [`umfutures_get_pm_account_info_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UmfuturesGetPmAccountInfoV1Error {
...
pub async fn umfutures_get_pm_account_info_v1(configuration: &configuration::Configuration, params: UmfuturesGetPmAccountInfoV1Params) -> Result<models::UmfuturesGetPmAccountInfoV1Resp, Error<UmfuturesGetPmAccountInfoV1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::UmfuturesGetPmAccountInfoV1Resp`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::UmfuturesGetPmAccountInfoV1Resp`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<UmfuturesGetPmAccountInfoV1Error> = serde_json::from_str(&content).ok();
