...
/// struct for passing parameters to the method [`get_futures_hist_data_link_v1`]
#[derive(Clone, Debug, Default)]
pub struct GetFuturesHistDataLinkV1Params {
...
/// struct for typed errors of method [`get_futures_hist_data_link_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetFuturesHistDataLinkV1Error {
...
pub async fn get_futures_hist_data_link_v1(configuration: &configuration::Configuration, params: GetFuturesHistDataLinkV1Params) -> Result<models::GetFuturesHistDataLinkV1Resp, Error<GetFuturesHistDataLinkV1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetFuturesHistDataLinkV1Resp`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetFuturesHistDataLinkV1Resp`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetFuturesHistDataLinkV1Error> = serde_json::from_str(&content).ok();
