...
/// struct for passing parameters to the method [`get_c2c_order_match_list_user_order_history_v1`]
#[derive(Clone, Debug, Default)]
pub struct GetC2cOrderMatchListUserOrderHistoryV1Params {
    pub timestamp: i64,
...
/// struct for typed errors of method [`get_c2c_order_match_list_user_order_history_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetC2cOrderMatchListUserOrderHistoryV1Error {
...
pub async fn get_c2c_order_match_list_user_order_history_v1(configuration: &configuration::Configuration, params: GetC2cOrderMatchListUserOrderHistoryV1Params) -> Result<models::GetC2cOrderMatchListUserOrderHistoryV1Resp, Error<GetC2cOrderMatchListUserOrderHistoryV1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetC2cOrderMatchListUserOrderHistoryV1Resp`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetC2cOrderMatchListUserOrderHistoryV1Resp`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetC2cOrderMatchListUserOrderHistoryV1Error> = serde_json::from_str(&content).ok();
