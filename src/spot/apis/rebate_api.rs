...
/// struct for passing parameters to the method [`get_rebate_tax_query_v1`]
#[derive(Clone, Debug, Default)]
pub struct GetRebateTaxQueryV1Params {
    pub timestamp: i64,
...
/// struct for typed errors of method [`get_rebate_tax_query_v1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetRebateTaxQueryV1Error {
...
pub async fn get_rebate_tax_query_v1(configuration: &configuration::Configuration, params: GetRebateTaxQueryV1Params) -> Result<models::GetRebateTaxQueryV1Resp, Error<GetRebateTaxQueryV1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetRebateTaxQueryV1Resp`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetRebateTaxQueryV1Resp`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetRebateTaxQueryV1Error> = serde_json::from_str(&content).ok();
